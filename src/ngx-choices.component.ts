...
export class NgxChoicesComponent implements OnInit, OnChanges, ControlValueAccessor{
  ...
  @HostListener('keydown', ['$event']) private onKeyDown(event) {
    // back button press
  ...
  // listen click inside
  @HostListener('click', ['$event']) private onClickInside(event) {
    this.focus();
  }

  // listen click outside
  @HostListener('document:click', ['$event']) private onClickOutside(event) {
  ...
  // init configuration
  private configInit(newConfig: any = {}) {
  ...
  private addValue(value, propagateChanges: boolean = true){
  ...
  private removeValue(value, index) {
  ...
  // filter remote items
  private getRemoteItems = _.debounce((query: string) => {
  ...
  private emitChanges(values: any) {
